...
object LabelTests extends TestSuite {
  val tests = Tests {
    ...
    'corner_boolean - {
      val label = Label(Label.props(corner = LabelCorner.Corner))
      ReactTestUtils.withRenderedIntoDocument(label) { m =>
        assert(m.outerHtmlScrubbed() == """<div class="ui corner label"></div>""")
      }
    }
    'corner_string - {
      val label = Label(Label.props(corner = LabelCorner.Left))
      ReactTestUtils.withRenderedIntoDocument(label) { m =>
    ...
    'detailProps - {
      val labelDetail =
        LabelDetail.props(className = "test", content = <.div("Detail"): VdomElement)
    ...
    'pointing - {
      val label = Label(Label.props(pointing = LabelPointing.Default))
      ReactTestUtils.withRenderedIntoDocument(label) { m =>
        assert(m.outerHtmlScrubbed() == """<div class="ui pointing label"></div>""")
      }
    }
    'pointingBelow - {
      val label = Label(Label.props(pointing = LabelPointing.Below))
      ReactTestUtils.withRenderedIntoDocument(label) { m =>
    ...
    'removeIcon - {
      val label = Label(Label.props(removeIcon = Icon(Icon.props(name = "remove"))))
      ReactTestUtils.withRenderedIntoDocument(label) { m =>
        assert(m.outerHtmlScrubbed() == """<div class="ui label"></div>""")
      }
    }
    'ribbonDefault - {
      val label = Label(Label.props(ribbon = LabelRibbon.Default))
      ReactTestUtils.withRenderedIntoDocument(label) { m =>
        assert(m.outerHtmlScrubbed() == """<div class="ui ribbon label"></div>""")
      }
    }
    'ribbonRight - {
      val label = Label(Label.props(ribbon = LabelRibbon.Right))
      ReactTestUtils.withRenderedIntoDocument(label) { m =>
